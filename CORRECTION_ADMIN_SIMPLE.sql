-- 🔧 CORRECTION ADMIN SIMPLE - CryptoBoost
-- ========================================

-- Script simplifié pour créer l'admin sans problème d'ID
-- Ce script utilise une approche plus sûre
...
BEGIN
    ...
    RAISE NOTICE 'Admin cree dans auth.users avec ID: %', new_admin_id;
    ...
    RAISE NOTICE 'Admin cree dans public.users avec ID: %', new_admin_id;
...
CREATE OR REPLACE FUNCTION test_admin_simple()
RETURNS text AS $$
DECLARE
    admin_count integer;
...
-- Exécuter le test
SELECT test_admin_simple();

-- Nettoyer
DROP FUNCTION test_admin_simple();
...
RAISE NOTICE 'Vous pouvez maintenant vous connecter a l''application';