-- Script complet de réparation de la base de données CryptoBoost (FINAL V3)
-- À exécuter dans l'éditeur SQL de Supabase
-- Ce script corrige TOUTES les erreurs identifiées et évite les duplications
-- Version 3 : Correction de l'erreur 42702 (ambiguïté de colonnes)
...
-- =====================================================
-- 7. CRÉATION DES INDEX POUR LES PERFORMANCES (SÉCURISÉE)
-- =====================================================

SELECT 'Création des index pour les performances (sécurisée)...' as status;

-- Fonction pour vérifier si une colonne existe (CORRIGÉE)
...
-- Index pour transactions (vérification des colonnes)
DO $$
BEGIN
  IF column_exists('transactions', 'user_id') THEN
    CREATE INDEX IF NOT EXISTS idx_transactions_user_id ON transactions(user_id);
  END IF;
  
  IF column_exists('transactions', 'status') THEN
    CREATE INDEX IF NOT EXISTS idx_transactions_status ON transactions(status);
  END IF;
  
  IF column_exists('transactions', 'type') THEN
    CREATE INDEX IF NOT EXISTS idx_transactions_type ON transactions(type);
  END IF;
  
  IF column_exists('transactions', 'created_at') THEN
    CREATE INDEX IF NOT EXISTS idx_transactions_created_at ON transactions(created_at);
  END IF;
END $$;

-- Index pour user_investments (vérification des colonnes)
DO $$
BEGIN
  IF column_exists('user_investments', 'user_id') THEN
    CREATE INDEX IF NOT EXISTS idx_user_investments_user_id ON user_investments(user_id);
  END IF;
  
  IF column_exists('user_investments', 'plan_id') THEN
    CREATE INDEX IF NOT EXISTS idx_user_investments_plan_id ON user_investments(plan_id);
  END IF;
  
  IF column_exists('user_investments', 'status') THEN
    CREATE INDEX IF NOT EXISTS idx_user_investments_status ON user_investments(status);
  END IF;
  
  IF column_exists('user_investments', 'start_date') THEN
    CREATE INDEX IF NOT EXISTS idx_user_investments_start_date ON user_investments(start_date);
  END IF;
END $$;

-- Index pour system_logs (vérification des colonnes)
DO $$
BEGIN
  IF column_exists('system_logs', 'level') THEN
    CREATE INDEX IF NOT EXISTS idx_system_logs_level ON system_logs(level);
  END IF;
  
  IF column_exists('system_logs', 'created_at') THEN
    CREATE INDEX IF NOT EXISTS idx_system_logs_created_at ON system_logs(created_at);
  END IF;
  
  IF column_exists('system_logs', 'user_id') THEN
    CREATE INDEX IF NOT EXISTS idx_system_logs_user_id ON system_logs(user_id);
  END IF;
  
  IF column_exists('system_logs', 'action') THEN
    CREATE INDEX IF NOT EXISTS idx_system_logs_action ON system_logs(action);
  END IF;
END $$;

-- Index pour notifications (vérification des colonnes)
DO $$
BEGIN
  ...
  IF column_exists('notifications', 'type') THEN
    CREATE INDEX IF NOT EXISTS idx_notifications_type ON notifications(type);
  END IF;
END $$;

-- Index pour users (vérification des colonnes)
DO $$
BEGIN
  IF column_exists('users', 'role') THEN
    CREATE INDEX IF NOT EXISTS idx_users_role ON users(role);
  END IF;
  
  IF column_exists('users', 'status') THEN
...
-- Index pour investment_plans (vérification des colonnes)
DO $$
BEGIN
  IF column_exists('investment_plans', 'is_active') THEN
    CREATE INDEX IF NOT EXISTS idx_investment_plans_is_active ON investment_plans(is_active);
  END IF;
  
  IF column_exists('investment_plans', 'created_at') THEN
    CREATE INDEX IF NOT EXISTS idx_investment_plans_created_at ON investment_plans(created_at);
  END IF;
END $$;

-- Index pour crypto_wallets (vérification des colonnes)
DO $$
BEGIN
  IF column_exists('crypto_wallets', 'is_active') THEN
    CREATE INDEX IF NOT EXISTS idx_crypto_wallets_is_active ON crypto_wallets(is_active);
  END IF;
  
  IF column_exists('crypto_wallets', 'symbol') THEN
...
SELECT '✅ Tous les index ont été créés (avec vérification des colonnes)' as info;
