-- 🔧 CORRECTION FINALE APRÈS NOUVELLES CLÉS - CryptoBoost
-- ======================================================

-- Ce script corrige les problèmes identifiés après la mise à jour des clés API
-- Problèmes détectés :
-- 1. "Database error querying schema" pour l'authentification admin
-- 2. Données dupliquées dans investment_plans
-- 3. Problèmes avec les wallets crypto

-- 1. Nettoyage des données dupliquées
-- ===================================

-- Supprimer les plans d'investissement dupliqués
...
VALUES 
    ('Starter', 'Plan d\'entrée pour débuter en crypto', 100, 30, NOW(), NOW()),
    ...
    ('Expert', 'Plan premium avec les meilleures stratégies et support dédié', 2000, 365, NOW(), NOW()),
    ('Pro', 'Plan pour les investisseurs sérieux avec des performances optimisées', 5000, 365, NOW(), NOW())
ON CONFLICT (name) DO NOTHING;

-- 2. Nettoyage des wallets crypto
-- ===============================

-- Supprimer les wallets dupliqués
...
-- Vérifier et corriger l'admin dans auth.users
DO $$
DECLARE
    admin_id uuid;
BEGIN
...
-- Recréer les politiques RLS simplifiées
