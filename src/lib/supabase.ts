...
export const userApi = {
  ...
  async getUserByEmail(email: string): Promise<User | null> {
    ...
    if (error) {
      logError('Error fetching user', error, 'userApi.getUserByEmail');
      return null;
    }
...
export const investmentApi = {
  ...
  async createPlan(planData: Omit<InvestmentPlan, 'id' | 'created_at'>): Promise<InvestmentPlan | null> {
    const { data, error } = await supabase
      .from('investment_plans')
      .insert([{
        ...planData,
  ...
  async updatePlan(planId: string, updates: Partial<InvestmentPlan>): Promise<InvestmentPlan | null> {
    const { data, error } = await supabase
      .from('investment_plans')
      .update(updates)
      .eq('id', planId)
